...
export default function LevelDetailPage({
      ...
      <div className="p-8 max-w-3xl mx-auto">
        <Card>
          <div className="text-center py-12">
            ...
            <p className="text-ink-2">找不到这个关卡：{resolvedId}</p>
            {onBack && (
              <div className="mt-4">
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      {/* 顶部导航 */}
      <div className="mb-4 flex items-center text-sm text-ink-2">
        <button
          onClick={onBack}
      ...
      <Card variant="elevated" className="mb-6 overflow-hidden p-0">
        ...
        <div className="flex items-start gap-6 p-6">
          <div className="w-24 h-24 rounded-xl bg-gradient-to-br from-accent-100 to-highlight/100 flex items-center justify-center text-5xl flex-shrink-0">
            {level.coverEmoji}
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xs font-mono text-accent-ink bg-accent-soft px-2 py-0.5 rounded">
                {level.id}
              </span>
            ...
            <p className="text-sm text-ink-2 mb-3">{level.description}</p>
      ...
      {/* 前置关卡提示 */}
      {prereqs.length > 0 && (
        <Card className="mb-6 bg-warning-soft border-warning-soft">
            ...
            <div className="text-sm text-warning">
              ...
              <div className="flex flex-wrap gap-2">
                {prereqs.map((p) => (
                  <span
                    key={p}
                    className="px-2 py-0.5 bg-surface border border-warning-soft rounded text-xs"
                  >
                    {p}
                  </span>
                ))}
          ...
          {level.steps.map((step) => (
            <Card key={step.id} variant="bordered">
              <div className="flex items-start gap-4">
                <div className="w-9 h-9 rounded-full bg-accent text-bg flex items-center justify-center font-semibold text-sm flex-shrink-0">
                  {step.orderNum}
                </div>
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-sm text-ink-2">{step.instruction}</p>
                  {step.placeholder && (
                    <div className="mt-2 text-xs text-ink-2 italic">
                      提示：{step.placeholder}
                    </div>
                  )}
                </div>
        ...
        <div className="grid grid-cols-5 gap-2">
          {Object.entries(level.scoringCriteria).map(([key, value]) => {
            ...
            return (
              <div
                key={key}
                className="text-center p-2 bg-surface-2 rounded-md"
              >
      ...
      {/* 开始按钮 */}
      <div className="sticky bottom-0 bg-bg -mx-8 px-8 py-4 border-t border-line">
