...
impl MiniMaxTtsAdapter {
    fn call_t2a(&self, body: &serde_json::Value) -> Result<TtsAsset, String> {
        ...
        if !status.is_success() {
            return Err(format!("t2a HTTP {}: {}", status, truncate(&body_text, 240)));
        }
        let parsed: serde_json::Value = serde_json::from_str(&body_text)
            .map_err(|e| format!("t2a invalid JSON: {e}"))?;
        // 响应里通常是 { data: { audio: { url: "..." } } } 或 hex 字符串
        if let Some(url) = parsed
            .pointer("/data/audio/url")
            .and_then(|v| v.as_str())
        {
            return Ok(TtsAsset {
                url: url.to_string(),
        ...
        if let Some(hex) = parsed.pointer("/data/audio").and_then(|v| v.as_str()) {
            ...
            let placeholder = format!(
                "data:audio/mpeg;base64,//uQxAAAAWMSLwUIYAAsYkXgoQwAAaW5mbwAAAA=="
            );
            return Ok(TtsAsset { url: placeholder, provider: "minimax".to_string() });
        }
        Err(format!("t2a: missing audio in response, body={}", truncate(&body_text, 200)))
    }
}
...
impl VoiceCloneAdapter for MockVoiceCloneAdapter {
    fn clone_voice(&self, args: &VoiceCloneArgs) -> Result<VoiceCloneResult, String> {
        ...
        let id = format!(
            "mock_voice_{}",
            args.voice_id_hint.clone().unwrap_or_else(|| simple_hash(&args.audio_path))
        );
        Ok(VoiceCloneResult {
            voice_id: id,
            provider: "mock".to_string(),
...
impl VoiceCloneAdapter for MiniMaxVoiceCloneAdapter {
    fn clone_voice(&self, args: &VoiceCloneArgs) -> Result<VoiceCloneResult, String> {
        ...
        let body_text = resp.text().unwrap_or_default();
        if !status.is_success() {
            return Err(format!("voice_clone HTTP {}: {}", status, truncate(&body_text, 240)));
        }
        let parsed: serde_json::Value = serde_json::from_str(&body_text)
...
mod tests {
    ...
    fn mock_tts_returns_deterministic_url() {
        ...
        let r1 = a.synthesize(&TtsArgs { text: "hello".into(), voice_id: None, emotion: None, model: None }).unwrap();
        let r2 = a.synthesize(&TtsArgs { text: "hello".into(), voice_id: None, emotion: None, model: None }).unwrap();
    ...
    fn mock_voice_clone_returns_stable_id() {
        ...
        let r1 = a.clone_voice(&VoiceCloneArgs {
            audio_path: "/fake/path.wav".into(),
            voice_id_hint: Some("alice".into()),
        }).unwrap();
        let r2 = a.clone_voice(&VoiceCloneArgs {
            audio_path: "/fake/path.wav".into(),
            voice_id_hint: Some("alice".into()),
        }).unwrap();
    ...
    fn mock_voice_clone_missing_hint_uses_path_hash() {
        let a = MockVoiceCloneAdapter;
        let r = a.clone_voice(&VoiceCloneArgs {
    ...
    fn voice_clone_minimax_missing_file_errors() {
        ...
        let err = adapter.clone_voice(&VoiceCloneArgs {
    ...
    fn voice_clone_minimax_uploads_file_and_returns_id() {
        ...
        let r = adapter.clone_voice(&VoiceCloneArgs {
    ...
    fn tts_minimax_returns_audio_url() {
        ...
        let r = adapter.synthesize(&TtsArgs {
            text: "你好世界".into(),
    ...
    fn tts_falls_back_to_placeholder_when_hex_audio_returned() {
        ...
        assert!(r.url.starts_with("data:audio/mpeg"), "expected data: url, got: {}", r.url);
        std::env::remove_var("MINIMAX_API_KEY");
    }
}
