...
GET /api/v1/asset-manifest
  → 读 assets/asset-manifest.json, 加 1h cache + ETag.
  → 返 {version, images: {key: full_url}}.

设计:
- 无 auth (公开只读, 资产是公开素材).
- 路径: ASSETS_DIR env 配 (默认 <project_root>/assets/asset-manifest.json).
...
def _manifest_path() -> Path:
    ...
    if raw:
        return Path(raw) / "asset-manifest.json"
    # 默认 <server 包目录>/../../assets/asset-manifest.json
    return Path(__file__).resolve().parents[2] / "assets" / "asset-manifest.json"
